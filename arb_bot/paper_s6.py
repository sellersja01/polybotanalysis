"""
S6: Penny Reversal (<=0.10) — Paper Trader
Buy when either side ask <= 0.10. Hold to resolution.
Low WR but massive wins when reversal happens.
"""
import asyncio, json, time, aiohttp, websockets
from datetime import datetime, timezone

SHARES=100;INTERVAL=300;MAX_ASK=0.10
...
async def main():
    print("="*70+"\n  S6: Penny Reversal (<=0.10) — All 4 markets\n"+"="*70)
    assets={}
    for cfg in ASSETS:
        assets[cfg["label"]]={**cfg,"candle_ts":0,"up_token":None,"dn_token":None,
            "up_bid":0.0,"up_ask":0.0,"dn_bid":0.0,"dn_ask":0.0,"question":"",
            "entered_up":False,"entered_dn":False,"trades":[],"last_cb_price":0.0,"candle_open_cb":0.0}
    ...
    def resolve(a):
        ...
        for t in a["trades"]:
            cost=(t["ask"]+fee(t["ask"]))*SHARES
            pnl=(SHARES-cost) if t["side"]==winner else (0-cost)
    ...
    async def setup(a):
        ...
        a["candle_ts"]=cs;a["entered_up"]=False;a["entered_dn"]=False;a["trades"]=[]
        a["up_bid"]=a["up_ask"]=a["dn_bid"]=a["dn_ask"]=0.0
        a["candle_open_cb"]=a.get("last_cb_price",0)  # snapshot at candle start
        up,dn,q,_=await get_market(a["slug"])
        if up and dn: a["up_token"]=up;a["dn_token"]=dn;a["question"]=q;print(f"[{a['label']}] {q}",flush=True)

    def check(a):
        if not a["entered_up"] and a["up_ask"]>0 and a["up_ask"]<=MAX_ASK:
            a["entered_up"]=True;a["trades"].append({"side":"Up","ask":a["up_ask"]})
            ts=datetime.now(timezone.utc).strftime("%H:%M:%S")
            print(f"  [{ts}] ENTRY [{a['label']}] Up @{a['up_ask']:.3f}",flush=True)
        if not a["entered_dn"] and a["dn_ask"]>0 and a["dn_ask"]<=MAX_ASK:
            a["entered_dn"]=True;a["trades"].append({"side":"Down","ask":a["dn_ask"]})
            ts=datetime.now(timezone.utc).strftime("%H:%M:%S")
            print(f"  [{ts}] ENTRY [{a['label']}] Down @{a['dn_ask']:.3f}",flush=True)
    ...
    async def status():
        while True:
            ...
            print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] S6 | {total_trades}t WR={wr:.0f}% PnL=${total_pnl:+.2f}",flush=True)
