"""
test_buy_sell2.py — Buy $1 of Up, wait 5s, sell it.
Fixed sell: subtracts 0.01 shares to stay under balance.

Usage:
    POLY_PRIVATE_KEY="0x..." python3 -u test_buy_sell2.py
"""
import asyncio
import json
import os
...
async def main():
    print("=" * 50)
    print("  BUY/SELL TEST v2 — fixed sell amount")
    print("=" * 50)
    ...
    # SELL — floor to 4 decimals then subtract 0.01
    sell_amount = floor(shares * 10000) / 10000 - 0.01
    if sell_amount <= 0:
        print(f"ERROR: sell_amount={sell_amount}, too small")
        return

    print(f"SELLING {sell_amount:.4f} shares (had {shares:.4f})...")
    t0 = time.perf_counter_ns()
    args = MarketOrderArgs(token_id=up_token, amount=sell_amount, side="SELL", price=0.01)
    signed = clob.create_market_order(args)
    resp = clob.post_order(signed, "FAK")
    ms = (time.perf_counter_ns() - t0) / 1e6
