"""
S13API: First CEX Move (API resolver) — Paper Trader
...
async def main():
    print("="*70+"\n  S13API: First CEX Move (0.03%) — API RESOLVER — All 4 markets\n"+"="*70)
    ...
    async def status():
        while True:
            ...
            print(f"[{ts}] S13API | {total_trades}t WR={wr:.0f}% PnL=${total_pnl:+.2f}",flush=True)
