"""
S12: Both Sides Cheap — Paper Trader
When Up ask + Down ask < 0.90, buy BOTH sides. Guaranteed $1.00 payout.
100% WR in backtest.
"""
import asyncio, json, time, aiohttp, websockets
from datetime import datetime, timezone

SHARES = 100; INTERVAL = 300
ASSETS = [
...
async def main():
    print("="*70+"\n  S12: Both Sides Cheap (< 0.90) — All 4 markets\n"+"="*70)
    assets={}
    for cfg in ASSETS:
    ...
    def resolve(a):
        ...
        print(f"  [{ts}] {tag} [{a['label']}] BOTH up={t['up_ask']:.3f}+dn={t['dn_ask']:.3f}={t['up_ask']+t['dn_ask']:.3f} pnl=${pnl:+.2f} | {total_trades}t WR={wr:.0f}% ${total_pnl:+.2f}",flush=True)
    ...
    def check(a):
        if a["entered"]: return
        if a["up_ask"]<=0.05 or a["dn_ask"]<=0.05: return
        combined=a["up_ask"]+a["dn_ask"]
        if combined<0.90:
            a["entered"]=True;a["trade"]={"up_ask":a["up_ask"],"dn_ask":a["dn_ask"],"ts":time.time()}
            ts=datetime.now(timezone.utc).strftime("%H:%M:%S")
            print(f"  [{ts}] ENTRY [{a['label']}] BOTH up={a['up_ask']:.3f}+dn={a['dn_ask']:.3f}={combined:.3f}",flush=True)
    ...
    async def status():
        while True:
            ...
            print(f"[{ts}] S12 | {total_trades}t WR={wr:.0f}% PnL=${total_pnl:+.2f}",flush=True)
