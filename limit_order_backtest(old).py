...
print(f"LIMIT ORDER MARKET MAKING BACKTEST")
print(f"Limit offset: -{LIMIT_OFFSET} below mid | {SHARES_PER_ORDER} shares/order | max {MAX_ORDERS} orders/side")
print(f"Fill condition: ask drops to our limit price within 60s")
print(f"{'='*70}\n")
...
for label, db_path in DBS.items():
    ...
    avg_fills = sum(r['n_up_fills'] + r['n_down_fills'] for r in result['results']) / max(len(result['results']), 1)

    print(f"{result['candles']} candles | WR={wr}% | PnL=${result['total_pnl']:+.2f} | avg/trade=${avg_pnl:+.2f} | avg_combined={avg_combined:.3f} | avg_fills={avg_fills:.1f}")
    all_results.append(result)

print(f"\n{'='*70}")
print(f"SUMMARY")
print(f"{'='*70}")
print(f"\n{'Market':<12} {'Candles':>8} {'WR':>6} {'Total PnL':>12} {'Avg/trade':>10} {'Avg Combined':>14} {'Avg Fills':>10}")
print(f"-"*75)
...
for r in all_results:
    ...
    both_leg = [x for x in r['results'] if not x.get('single_leg')]
    single_leg = [x for x in r['results'] if x.get('single_leg')]
    avg_combined = sum(x['combined'] for x in both_leg) / max(len(both_leg), 1)
    avg_fills = sum(x['n_up_fills'] + x['n_down_fills'] for x in r['results']) / max(len(r['results']), 1)
    both_pnl = sum(x['pnl'] for x in both_leg)
    single_pnl = sum(x['pnl'] for x in single_leg)
    marker = " ✓" if r['total_pnl'] > 0 else ""
    print(f"{r['label']:<12} {total:>6} {wr:>5}% {r['total_pnl']:>+10.2f} | both={len(both_leg)}(${both_pnl:+.0f}) single={len(single_leg)}(${single_pnl:+.0f}) | comb={avg_combined:.3f}{marker}")

print(f"\nKey: avg_combined < 1.00 = profitable | > 1.00 = losing")
print(f"If fills are low, LIMIT_OFFSET is too aggressive")
print(f"Try adjusting LIMIT_OFFSET between 0.01 and 0.05")
