...
RISK_PCT          = 0.04       # 4% of balance per trade (compounds)
MAX_SIMULTANEOUS  = 10
CUT_THRESHOLD     = 0.05       # exit early if mid drops to this
MIN_ENTRY         = 0.10       # min odds to enter
MAX_ENTRY         = 0.90       # max odds to enter

# Thresholds in odds-scale units (0.0-1.0)
THRESH_SMALL = 0.04   # S1/S7 trigger — odds move 4 cents in first 2 min
THRESH_LARGE = 0.10   # S2 trigger    — odds move 10 cents in first 2 min
S7_SPREAD_MAX = 0.015 # S7 requires tight spread
...
def get_signal(asset, tf, candle_id, outcome, mid, ask, bid):
    ...
    # odds-scale move in first 2 minutes
    ...
    # ── DOWN strategies ──────────────────────────────────────────
    if outcome == "Down":
        # S2 Down: strong momentum down (large odds move)
        if move_2m < -THRESH_LARGE:
            return "S2_DOWN"

        # S1 Down: early momentum down AND odds still cheap (under 0.45)
        if move_2m < -THRESH_SMALL and mid < 0.45:
            return "S1_DOWN"

        # S7 Down: tight spread + momentum down AND odds cheap
        if spread < S7_SPREAD_MAX and move_2m < -THRESH_SMALL and mid < 0.45:
            return "S7_DOWN"

    # ── UP strategies ────────────────────────────────────────────
    if outcome == "Up":
        # S1 Up: strong momentum up AND odds still cheap (under 0.45)
        if move_2m > THRESH_LARGE and mid < 0.45:
            return "S1_UP"
...
async def try_enter(asset, tf, candle_id, outcome, mid, ask, bid, signal):
    async with lock:
        ...
        bet    = min(account["balance"] * 0.04, account["balance"])  # 4% per trade
        shares = bet / ask
        spread = ask - bid
...
async def main():
    init_log_db()
    print(f"\n PAPER TRADER v4 — Starting balance: ${STARTING_BALANCE:,.2f}")
    print(f"  Risk: 4% per trade | Max positions: {MAX_SIMULTANEOUS}")
    print(f"  Strategies: S1_DOWN S2_DOWN S7_DOWN S1_UP")
