"""
Capture tick-by-tick odds for a specific 5m candle plus any bot signals,
and export as a CSV spreadsheet.

Reads collector DB + live_s13.log for a given [cs, ce) UTC window.
Outputs /tmp/candle_YYYYMMDD_HHMM.csv — one row per collector tick and
one row per bot ENTRY event, merged in chronological order.

Columns:
  timestamp_utc, millis_since_candle_start, event_type, asset, side,
  up_bid, up_ask, up_mid, dn_bid, dn_ask, dn_mid, entry_log_ask, entry_log_mid, entry_mv_pct
"""
import sqlite3, re, csv, os
from datetime import datetime, timezone, timedelta

# ──────────────────────────────────────────────────────────────
# Candle window (UTC) — edit these two lines if you want a different candle
CANDLE_START = datetime(2026, 4, 21, 18, 15, tzinfo=timezone.utc)
CANDLE_END   = datetime(2026, 4, 21, 18, 20, tzinfo=timezone.utc)
# ──────────────────────────────────────────────────────────────
...
# 1. Collector ticks — per asset
for asset in ASSETS:
    src = f'/root/market_{asset}_5m.db'
    snap = f'{OUT_DIR}/snap_cs_{asset}.db'
    backup(src, snap)
    ...
    # Pivot: for each unix_time we may have an Up row OR Down row (not both)
    # Track running up/dn state as we walk forward
    up_bid = up_ask = dn_bid = dn_ask = None
    for ut, outcome, bid, ask in rows:
        ut = float(ut); bid = float(bid); ask = float(ask)
        if outcome == 'Up':
            up_bid, up_ask = bid, ask
        else:
            dn_bid, dn_ask = bid, ask
...
# 2. Bot ENTRY events from the log
entry_re = re.compile(
    ...
    if em:
        ...
        if cs_ts <= ut < ce_ts:
            all_rows.append({
                ...
                'dt': dt,
                'ms_since_start': (ut - cs_ts) * 1000,
                'event_type': f'ENTRY_{em.group(5)}',  # ENTRY_DRY or ENTRY_LIVE
                'asset': em.group(6),
                'side': em.group(7),
...
# 3. Sort chronologically, write CSV
...
n_entries = sum(1 for r in all_rows if r['event_type'].startswith('ENTRY'))
print(f'Wrote {out_path}')
print(f'  total rows: {len(all_rows)}')
print(f'  collector ticks: {n_ticks}')
print(f'  bot ENTRY events: {n_entries}')
print(f'  candle window: {CANDLE_START.strftime("%H:%M:%S")} - {CANDLE_END.strftime("%H:%M:%S")} UTC')
if n_entries:
    print('\n  Bot entries in this candle:')
    for r in all_rows:
        if r['event_type'].startswith('ENTRY'):
            print(f'    t+{r["ms_since_start"]:7.0f}ms | {r["event_type"]} [{r["asset"]}] {r["side"]} @{r["entry_log_ask"]} mid={r["entry_log_mid"]} mv={r["entry_mv_pct"]}%')
