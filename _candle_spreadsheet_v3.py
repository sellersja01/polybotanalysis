"""Same as _candle_spreadsheet.py but reads from live_s13_v3.log instead of live_s13.log,
so we can verify v3's entry book view matches the collector."""
import sqlite3, re, csv, os
from datetime import datetime, timezone, timedelta

CANDLE_START = datetime(2026, 4, 21, 18, 45, tzinfo=timezone.utc)
CANDLE_END   = datetime(2026, 4, 21, 18, 50, tzinfo=timezone.utc)

ASSETS = ['btc', 'eth', 'sol', 'xrp']
LOG    = '/root/live_s13_v3.log'
OUT_DIR = '/tmp'

cs_ts = CANDLE_START.timestamp()
ce_ts = CANDLE_END.timestamp()
tag   = 'v3_' + CANDLE_START.strftime('%Y%m%d_%H%M')

def backup(src_path, dst_path):
    a = sqlite3.connect(src_path, timeout=30)
    b = sqlite3.connect(dst_path)
    a.backup(b); a.close(); b.close()

all_rows = []

for asset in ASSETS:
    src = f'/root/market_{asset}_5m.db'
    snap = f'{OUT_DIR}/snap_v3_{asset}.db'
    backup(src, snap)
    c = sqlite3.connect(snap)
    rows = c.execute(
        "SELECT unix_time, outcome, bid, ask FROM polymarket_odds "
        "WHERE unix_time >= ? AND unix_time < ? AND outcome IN ('Up','Down') "
        "ORDER BY unix_time",
        (cs_ts, ce_ts)).fetchall()
    c.close()
    os.remove(snap)
    up_bid=up_ask=dn_bid=dn_ask=None
    for ut, outcome, bid, ask in rows:
        ut=float(ut); bid=float(bid); ask=float(ask)
        if outcome == 'Up': up_bid, up_ask = bid, ask
        else:               dn_bid, dn_ask = bid, ask
        up_mid = (up_bid+up_ask)/2 if up_bid and up_ask else None
        dn_mid = (dn_bid+dn_ask)/2 if dn_bid and dn_ask else None
        all_rows.append({
            'ts': ut, 'dt': datetime.fromtimestamp(ut, tz=timezone.utc),
            'ms_since_start': (ut - cs_ts) * 1000,
            'event_type': 'TICK', 'asset': asset.upper(), 'side': outcome,
            'up_bid': up_bid, 'up_ask': up_ask, 'up_mid': up_mid,
            'dn_bid': dn_bid, 'dn_ask': dn_ask, 'dn_mid': dn_mid,
            'entry_log_ask': '', 'entry_log_mid': '', 'entry_mv_pct': '',
        })

entry_re = re.compile(
    r'\[(\d+):(\d+):(\d+)\.(\d+)\] ENTRY\[(DRY|LIVE)\] \[(\w+)\] (\w+) @([\d.]+) mid=([\d.]+) mv=([+-][\d.]+)%')

with open(LOG) as f:
    log_lines = f.readlines()
cur_date = datetime.now(timezone.utc).date()
prev_h = None
for line in reversed(log_lines):
    m = re.search(r'\[(\d+):(\d+):(\d+)', line)
    if not m: continue
    h = int(m.group(1))
    if prev_h is not None and h > prev_h: cur_date -= timedelta(days=1)
    prev_h = h
    em = entry_re.search(line)
    if em:
        h,mi,s,frac = em.group(1), em.group(2), em.group(3), em.group(4)
        micros = int(frac.ljust(6,'0')[:6])
        dt = datetime.combine(cur_date, datetime.min.time(), tzinfo=timezone.utc).replace(
            hour=int(h),minute=int(mi),second=int(s),microsecond=micros)
        ut = dt.timestamp()
        if cs_ts <= ut < ce_ts:
            all_rows.append({
                'ts': ut, 'dt': dt, 'ms_since_start': (ut-cs_ts)*1000,
                'event_type': f'ENTRY_{em.group(5)}_V3',
                'asset': em.group(6), 'side': em.group(7),
                'up_bid':'', 'up_ask':'', 'up_mid':'',
                'dn_bid':'', 'dn_ask':'', 'dn_mid':'',
                'entry_log_ask': float(em.group(8)),
                'entry_log_mid': float(em.group(9)),
                'entry_mv_pct': float(em.group(10)),
            })

all_rows.sort(key=lambda r: r['ts'])
out_path = f'{OUT_DIR}/candle_{tag}.csv'
with open(out_path,'w',newline='') as f:
    w = csv.writer(f)
    w.writerow(['timestamp_utc','ms_since_candle_start','event_type','asset','side',
                'up_bid','up_ask','up_mid','dn_bid','dn_ask','dn_mid',
                'entry_log_ask','entry_log_mid','entry_mv_pct'])
    for r in all_rows:
        w.writerow([
            r['dt'].strftime('%H:%M:%S.%f')[:-3],
            f"{r['ms_since_start']:.0f}",
            r['event_type'], r['asset'], r['side'],
            f"{r['up_bid']:.4f}" if isinstance(r['up_bid'],float) else '',
            f"{r['up_ask']:.4f}" if isinstance(r['up_ask'],float) else '',
            f"{r['up_mid']:.4f}" if isinstance(r['up_mid'],float) else '',
            f"{r['dn_bid']:.4f}" if isinstance(r['dn_bid'],float) else '',
            f"{r['dn_ask']:.4f}" if isinstance(r['dn_ask'],float) else '',
            f"{r['dn_mid']:.4f}" if isinstance(r['dn_mid'],float) else '',
            r['entry_log_ask'], r['entry_log_mid'], r['entry_mv_pct'],
        ])

n_ticks   = sum(1 for r in all_rows if r['event_type']=='TICK')
n_entries = sum(1 for r in all_rows if 'ENTRY' in r['event_type'])
print(f'Wrote {out_path}')
print(f'  collector ticks: {n_ticks}  |  v3 bot entries: {n_entries}')
print(f'  candle {CANDLE_START.strftime("%H:%M")} - {CANDLE_END.strftime("%H:%M")} UTC')
if n_entries:
    print('\n  v3 ENTRY events — compare each to nearest collector TICK of same asset:')
    # For each entry, find the bracket TICK rows of the same asset
    for ei, r in enumerate(all_rows):
        if 'ENTRY' not in r['event_type']: continue
        asset = r['asset']
        # Walk backward for nearest same-asset TICK
        before = None
        for rr in reversed(all_rows[:ei]):
            if rr['event_type']=='TICK' and rr['asset']==asset:
                before = rr; break
        # Walk forward
        after = None
        for rr in all_rows[ei+1:]:
            if rr['event_type']=='TICK' and rr['asset']==asset:
                after = rr; break
        print(f'\n  t+{r["ms_since_start"]:6.0f}ms  ENTRY_V3 [{asset}] {r["side"]} @{r["entry_log_ask"]} mid={r["entry_log_mid"]} mv={r["entry_mv_pct"]}%')
        if before:
            side_label = 'up' if r['side']=='Up' else 'dn'
            print(f'    nearest TICK BEFORE (-{r["ms_since_start"]-before["ms_since_start"]:.0f}ms): {side_label}_mid={before[f"{side_label}_mid"]:.3f} {side_label}_ask={before[f"{side_label}_ask"]:.3f}')
        if after:
            side_label = 'up' if r['side']=='Up' else 'dn'
            print(f'    nearest TICK AFTER  (+{after["ms_since_start"]-r["ms_since_start"]:.0f}ms): {side_label}_mid={after[f"{side_label}_mid"]:.3f} {side_label}_ask={after[f"{side_label}_ask"]:.3f}')
