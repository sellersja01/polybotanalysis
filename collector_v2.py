...
def init_db(db_path):
    ...
    conn.execute("""
        CREATE TABLE IF NOT EXISTS polymarket_odds (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            timestamp TEXT, unix_time REAL, market_id TEXT,
...
async def stream_price(asset):
    coinbase_id = PRICE_ASSETS[asset]
    label = f"[{asset.upper()}]"
    while True:
        try:
            ...
            print(f"{label} Price error: {e} - reconnecting in {delay:.1f}s...")
    ...
    while not stop_event.is_set():
        try:
            ...
            if not stop_event.is_set():
                delay = random.uniform(3, 10)
                print(f"{label} WS error: {e} - reconnecting in {delay:.1f}s...")
                await asyncio.sleep(delay)
...
async def watchdog():
    ...
    while True:
        ...
        for asset, tf in MARKETS:
            try:
                conn = sqlite3.connect(get_db_path(asset, tf))
                row = conn.execute("SELECT MAX(unix_time) FROM polymarket_odds").fetchone()
                conn.close()
                if row and row[0]:
                    most_recent = max(most_recent, float(row[0]))
            except:
                pass
        if most_recent > 0:
            age = now - most_recent
...
async def main():
    ...
    print(f"\nCollector v2 -- 8 markets: BTC/ETH/SOL/XRP 5m+15m\n")
