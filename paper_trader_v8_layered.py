"""
Paper Trader v8 Layered — Wait-for-Divergence, All 5 Entry Levels

Markets: BTC_5m, BTC_15m, ETH_5m  (ETH_15m excluded — negative ROI in backtest)
Strategy:
  - Buy BOTH sides every time either mid crosses a new threshold: 0.45, 0.40, 0.35, 0.30, 0.25
  - Each level = SHARES per side at current ask prices (up to 5 entries per candle)
  - If either side's mid drops to 0.20, early exit ALL shares of that side at bid = 2*mid - ask
...
Backtest results (100% candles, real fees, 3-day sample):
  BTC_5m:  76.6% WR, 2.89% ROI, +$13.07/candle @ 100 shares/level
  BTC_15m: 77.1% WR, 0.39% ROI, +$1.80/candle  @ 100 shares/level
  ETH_5m:  77.1% WR, 0.20% ROI, +$0.93/candle  @ 100 shares/level

Run:  python paper_trader_v8_layered.py
"""
...
# ── Config ──────────────────────────────────────────────────────────────────────
ENTRY_LEVELS = [0.45, 0.40, 0.35, 0.30, 0.25]   # triggers in order from highest to lowest
EXIT_MID     = 0.20    # early exit ALL shares of a side when its mid drops here
SHARES       = 100     # shares per side per level
PAPER_DB     = '/home/opc/paper_v8_layered.db'
...
def log_fill(market, candle_start, market_id, outcome, level, ask):
    conn = sqlite3.connect(PAPER_DB)
    conn.execute("""
        INSERT INTO fills (ts, market, candle_start, market_id, outcome, level, ask, shares, cost)
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (time.time(), market, candle_start, market_id, outcome, level, ask, SHARES, ask * SHARES))
    conn.commit()
    conn.close()
...
def save_resolved(market, candle_start, market_id, winner, up_fills, dn_fills,
                  up_exit_bid, dn_exit_bid, levels_hit):
    ...
    conn.execute("""
        INSERT OR IGNORE INTO resolved
          (resolved_at, market, candle_start, market_id, winner, levels_hit,
           up_shares, dn_shares, up_cost, dn_cost, avg_up, avg_dn,
           up_exit_bid, dn_exit_bid, pnl, win)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """, (time.time(), market, candle_start, market_id, winner, levels_hit,
    ...
    print(f"  [{icon}] {market:<10} {ts_str} | winner={winner} | lvls={levels_hit} | "
            ...
            async with websockets.connect(
                ...
                while not stop_evt.is_set():
                    ...
                    for ev in events:
                        ...
                        new_entries = []   # list of (up_ask, dn_ask, level)
                        do_exit     = None
                        ...
                        async with state_lock:
                            ...
                            # ── Entry triggers (one per level per candle) ──
                            if outcome == 'Up':
                            ...
                            if up_ask and dn_ask:
                                for lvl in ENTRY_LEVELS:
                                    if lvl not in s['levels_triggered'] and mid <= lvl:
                                        s['levels_triggered'].add(lvl)
                                        new_entries.append((up_ask, dn_ask, lvl))

                            # ── Early exit trigger ─────────────────────────
                            if s['levels_triggered'] and mid <= EXIT_MID:
                        ...
                        for up_ask, dn_ask, lvl in new_entries:
                            log_fill(market_name, candle_start, market_id, 'Up',   lvl, up_ask)
                            log_fill(market_name, candle_start, market_id, 'Down', lvl, dn_ask)
                            print(f"  ENTRY [{lvl:.2f}] {market_name:<10} Up@{up_ask:.3f} Dn@{dn_ask:.3f} "
                                  f"| mid={mid:.3f} | {ts_str}")

                        if do_exit:
                            side, exit_bid = do_exit
                            print(f"  EXIT  {market_name:<10} {side} ALL shares @ bid={exit_bid:.3f} "
                                  f"| mid={mid:.3f} | {ts_str}")
        ...
        if candle_start != prev_candle_start:
            ...
            if prev_candle_start is not None:
                async with state_lock:
                    ...
                    lvls_hit   = len(s.get('levels_triggered', set()))
                ...
                if up_fills or dn_fills:
                    ...
                    save_resolved(market_name, prev_candle_start, mid_id,
                                  winner, up_fills, dn_fills, up_exit, dn_exit, lvls_hit)
...
async def print_stats():
    while True:
        ...
        try:
            ...
            """).fetchone()
            conn.close()
            n, pnl, dep, wins, avg_lvls = row
            n = n or 0; pnl = pnl or 0; dep = dep or 0; wins = wins or 0; avg_lvls = avg_lvls or 0
            wr  = 100 * wins / n if n else 0
            roi = 100 * pnl  / dep if dep else 0
            print(f"\n  === {datetime.now(timezone.utc).strftime('%H:%M UTC')} | "
                  f"Candles: {n} | WR: {wr:.1f}% | PnL: ${pnl:+.2f} | "
                  f"ROI: {roi:.2f}% | AvgLevels: {avg_lvls:.1f} ===\n")
        except Exception:
            pass
...
async def main():
    print(f"\n{'='*65}")
    print(f"  PAPER TRADER v8 LAYERED — All 5 Entry Levels")
    print(f"  Levels: {ENTRY_LEVELS} | Early exit: mid <= {EXIT_MID}")
    print(f"  Shares: {SHARES}/side/level | Markets: BTC_5m, BTC_15m, ETH_5m")
    print(f"  DB: {PAPER_DB}")
